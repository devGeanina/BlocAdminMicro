	...
	public String creatExpenseSummary(List<ExpenseDTO> expenses) {
		...
		if(expenses != null && !expenses.isEmpty()) {
			List<ExpenseDTO> monthlyExpenses = expenses.stream()
				    .filter(o -> o.getExpenseTypeEnum().getName().equalsIgnoreCase(ExpenseType.MONTHLY.getName())).collect(Collectors.toList());
			List<ExpenseDTO> yearlyExpenses = expenses.stream()
				    .filter(o -> o.getExpenseTypeEnum().getName().equalsIgnoreCase(ExpenseType.YEARLY.getName())).collect(Collectors.toList());
			double leftoverSum = expenses.stream().mapToDouble(o -> o.getLeftoverSum()).sum();
			builder.append("There are a total of ").append(expenses.size()).append(" expenses registered, out of which ").append(monthlyExpenses.size()).append(" monthly and ").append(yearlyExpenses.size()).append(" yearly. Leftover sum to recover from tenants: ").append(leftoverSum).append(".");
		}else {
